...
**Interfaces:**
- `trade_api/schemas.py`: `def kr_int_str(v: Decimal | None) -> Decimal | None` — `v == v.to_integral_value()` 이면 `Decimal(int(v))`, 아니면 그대로(`"70000.50"` 유지). **`normalize()` 금지** — `Decimal("70000.0").normalize()` 는 `7E+4` 라 문자열이 `"7E+4"` 가 된다(컨트롤러 실측). `"1E+4"` 입력도 `"10000"` 으로. `QuoteOut`·`SellableQuantityResponse` 를 감싸는 출력 모델 대신 **라우터에서 값 정규화 후 모델 생성**: `quote()` 는 `price.lastPrice`, `limits.upperLimitPrice/lowerLimitPrice` 를 `kr_int_str` 로; `sellable()` 는 `sellableQuantity` 를; `holdings()` 는 `items[].quantity/lastPrice/averagePurchasePrice` 를 (KR 항목만).
