...
**(A2 종결 판정 — 전문가, 2026-09-02)**: 잔여 4조건 구성 조정 불필요. book fidelity
관점에서 **개선**으로 판정. 근거:
- 돌파 강도 정량 게이트가 volume_band 단독인 것은 **O'Neil HMMS 의 원 설계와 일치**.
  제거 전 스프레드 축은 거래량과 corr +0.578 로 **동일 현상의 중복 측정**이었으므로
  축 상실이 아니다.
- 당일 봉 질 검사는 close_upper_third 가 계속 담당 — book 근거: TLOND(mid-bar 마감=약함),
  Minervini(종가의 레인지 내 위치).
- "늦은 진입" 방어는 extended 인터셉트 1.05× 가 담당하며 **book-mandated** —
  O'Neil HMMS: 정확한 매수점에서 5~10% 이상 지나 사면 늦으며 8% 손절 규칙에 걸려 정상
  조정을 못 견딤(확장 5% 초과 시 추격·불타기 금지); Minervini TLSMW/TTLC: 피봇에서 몇
  퍼센트 이상 추격 금지. → 제거한 스프레드 게이트가 어설프게 겸하던 역할이며, 책 근거를
  갖춘 전담 장치가 이미 존재했음을 확인.

**(승인 확정) D3 재량 결정 승인 — 근거 보강**: spread_ratio_vs_avg 는 저장 OHLC 에서
재계산 가능한 파생값이므로 기록 제거에 정보 손실이 없다. 일반 규약으로 승격 —
governance.md 원칙 1-2(파생값 제거 가능 / 재계산 불가 원시 관측은 기록 보존).

## 제거 판정 기준 4조건 (전문가 확정, 코드로 재확인)

> 2026-09-03 governance.md 원칙 1-1 로 승격(일반 규약). 아래는 #151 적용 기록 원문.
