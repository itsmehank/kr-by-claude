...
| 유니버스 후 | (Task 6 Step 5) 기대 2,554 |
| 편입 3종목 | 088980 맥쿼리인프라(사회간접자본투융자회사·구분 내 일관성) · 138040 메리츠금융지주(주권·오탐 수정) · 369370 블리츠웨이엔터테인먼트(주권·오탐 수정) |
| 자격 대상(전문가 정의 = 활성 − 게이트 제외 3) | 기대 2,551 |
| 자격 대상(UNRESOLVED 도 제외한 실 게이트 통과) | 기대 2,550 |
| UNRESOLVED 건수 | 기대 1 (096610 알에프세미 — 공매도 응답 부재, 거래 재개 후 해결) |
| 배제 집합 차분 4건 반영 | −088980 −138040 −369370(배제 해제) / 094800·415640 = 적재 전 배제 아님(행 유지) → 스냅샷 211 |
| 소급 표기 | weekly_classification 6 / trigger_evaluation_log 1 / entry_params 1 (+094800 종료 행 1, [Q-6]) |
| 지표 전진 적용 | 09-15 이후 첫 incremental 후 확인 — 094800·415640·088980 NULL, 과거 TRUE 246·48 불변 |
| 백테스트 파리티 | armA-prod 1.1676 · exits 37 (불변 기대) |
| 138040 시계열 수집 | Task 6 Step 7 승인 결과에 따라 기록 |
| suite | (Task 8 Step 1) |

## 7. 이름 3축 경계 점검 결과(보고 전용 — 이번 스프린트 수정 금지)

(Task 4 Step 7 출력 요약. 사전 관측: 저장본 `only_in_krx` 샘플에 138930 BNK금융지주(주권) 가 있었고 `ETF_PREFIXES` 에 `"BNK"` 가 있어 접두사 오탐 확정 후보.)
