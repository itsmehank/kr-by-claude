...
| `kr_pipeline/llm_runner/compute/failed_breakout.py` (신규) | `compute_failed_breakout(conn, symbol, classified_at, pivot_price, base_start_date)` — K=5 + 지속성 (base 범위 한정) |
...
git commit -m "feat(phase1-2a): 2-F failed_breakout 계산 (K=5 + 지속성, base_start 범위 한정)

탐색 범위 = [base_start_date, classified_at) — 이번 base 의 D0 만 평가.
전체 역사 탐색은 과거 무관 돌파를 잡는 버그 (사용자 v5). base_start NULL → None.
D0=close>=pivot 최초일. D1~D5 내:
(P1) 연속 2일 이상 close<pivot OR (P2) 회복 0회.
단일일 throwback 보호. trigger='P1'|'P2'|'both' 명시.
테스트 6건: P1/P2 발화 + throwback/돌파없음 미발화 + base_start window/NULL.
재조정 대상 (사례 1건 기반 시작값)."
```
...
apply_phase1_gates: handle_quality 발화 시 risk_flags 주입 (관찰, classification 무관).
2-E 강등은 classification=='entry' 일 때만:
- Tier 1 (단독) → watch + conf≤0.60. Tier 2 (+extended_from_ma) → watch + conf≤0.50.
- watch/ignore 는 flag 만 추가, 강등/승격 없음 (ignore→watch 승격 버그 차단).
2-F 는 강등 없이 triggered_rules 기록만 (base_start 범위 전달). 발화 0건이면 None.
테스트 6건: tier1/tier2/미발화/2F기록 + ignore유지 + watch무변경."
```
...
- `compute_failed_breakout(conn, symbol, classified_at, pivot_price, base_start_date)` — Task 3 정의, Task 4 gates 가 result['base_start_date'] 전달 ✓
- `apply_phase1_gates(conn, symbol, classified_at, result) -> (result, triggered_rules)` — Task 4 정의, Task 5 store 에서 동일 호출 ✓
- **버그 수정 일관성 (사용자 v5)**: ① 휴리스틱 cup_bottom→right_rim (Task 2 + 앵커 handle_low 검증), ② 2-F base_start 범위 (Task 3 + window 테스트), ③ 2-E entry-only (Task 4 + ignore/watch 유지 테스트) ✓
