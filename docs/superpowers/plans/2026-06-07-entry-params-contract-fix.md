...
### Task 3: `insert_entry_params` 정규화 사용 + 신규 5컬럼 INSERT + 라운드트립 테스트
...
- [ ] **Step 4: 통과 확인** — `uv run pytest tests/test_llm_runner_store.py -k "entry_params or normalize" -v` → 라운드트립 + normalize 단위 PASS.
...
- [ ] **Step 5: 통과 확인** — `uv run pytest tests/test_llm_runner_store.py -k "mock or normalize or entry_params" -v` → PASS.

- [ ] **Step 6: 커밋**

```bash
git add kr_pipeline/llm_runner/llm/claude_cli.py kr_pipeline/llm_runner/entry_params.py tests/test_llm_runner_store.py
