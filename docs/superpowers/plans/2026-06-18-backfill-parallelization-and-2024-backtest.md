...
(주의: `reap_stale_weekend_runs`, `_already_classified`, `_process_one` 정의는 그대로 유지.
`call_claude`는 `_process_one`이 사용하므로 유지. **`ClaudeCLIError`는 이식 후 weekend 코드에서
미사용**(현재도 line 66 주석에서만 언급)이라 import에서 제거. `import psycopg`(모듈)도 이식 후
미사용이라 제거하되 `from psycopg import Connection`(타입힌트)은 유지.)

(b) `run()` 본문에서 **`concurrency = concurrency or ...` 줄부터 함수 끝 `return` 까지**(현재의
ThreadPoolExecutor 블록 전체)를 아래로 교체:
...
`tests/test_llm_backfill.py` 끝에 추가. **메시지를 검증**해야 유효한 TDD가 됨 — 구현 전에는
argparse가 "unrecognized arguments"로 SystemExit 하므로 단순 `pytest.raises(SystemExit)`만으론
이미 통과(틀린 이유). 우리 가드 메시지를 확인해 fail-before / pass-after를 보장:
...
Expected: FAIL — 인자가 아직 없어 argparse가 "unrecognized arguments: --concurrency=4"로 종료하므로
가드 메시지 assert가 불일치로 실패.
...
`DONE backfill: {... "processed": <합계>, "weeks": 52 ...}`. 8종목 합계 통과 ≈ 191회 분의
inline+차트 빌드가 성공(insert는 dry-run이라 생략). 동시 4 워커가 차트를 병렬 렌더링하나,
weekend가 이미 동일 방식으로 운영 중이라 thread-safe 검증됨(신규 리스크 아님). 차트 빌드만
~191회라 수 분 소요 가능.
...
- [ ] **Step 0: 기존 backfill 데이터 확인(resume 오해 방지)**

Run:
```bash
psql postgresql://localhost/kr_pipeline -c "
SELECT symbol, COUNT(*) FROM classification_backfill
WHERE analyzed_for_date BETWEEN '2024-01-06' AND '2024-12-28'
  AND symbol IN ('003230','101930','399720','200470','257720','000320','900340','267260')
GROUP BY symbol;"
```
Expected: 0행(클린). 행이 있으면 과거 실험분 — 그대로 두면 resume으로 skip되니, 새로 받고
싶으면 해당 (symbol, 2024 토요일) 행을 DELETE 후 진행.
