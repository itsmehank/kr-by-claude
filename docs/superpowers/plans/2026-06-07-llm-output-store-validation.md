...
Expected: FAIL — 현재는 검증 없이 gates/cursor 로 진행 → ValueError 안 남(또는 다른 에러). (insert_classification 은 gates 에서 conn.cursor 접근 시도; insert_trigger_log 는 result["decision"] KeyError.)
...
- 거부=ValueError(호출부 try/except 로그+rollback): Task 2 와이어링 테스트(mock conn, cursor 미접근) ✓
