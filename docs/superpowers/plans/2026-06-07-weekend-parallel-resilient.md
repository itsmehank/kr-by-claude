...
- Test: `tests/test_runs.py` (기존 파일에 **추가** — run_tracking 테스트가 이미 여기 있음)

> 주의: `db` 픽스처는 ROLLBACK 으로 격리하지만 `run_tracking` 은 **자체 commit** 하므로 행이 잔류한다. 기존 `tests/test_runs.py` 패턴대로 테스트 끝에 `DELETE FROM pipeline_runs ... + db.commit()` 로 직접 정리한다.

- [ ] **Step 1: 실패 테스트 작성** — `tests/test_runs.py` 에 추가:
...
def test_run_tracking_marks_failed_on_keyboardinterrupt(db):
    ...
    # run_tracking 의 실패 UPDATE 는 자체 commit → db 는 READ COMMITTED 로 최신 상태 조회
    ...
    with db.cursor() as cur:                      # cleanup (자체 commit 분 직접 정리)
...
- [ ] **Step 2: 실패 확인** — `uv run pytest tests/test_runs.py -k keyboardinterrupt -v`
...
- [ ] **Step 4: 통과 확인** — `uv run pytest tests/test_runs.py -v` → 신규 2 + 기존 4 passed. (기존 run_tracking 테스트도 통과 — 성공/예외 동작 동일, 단 BaseException 으로 확대.)
...
- Test: `tests/test_llm_weekend.py` (기존 weekend 테스트 파일에 **추가**)

> 주의: `_seed_run`·reaper 는 commit 하므로 `db` 롤백으로 정리 안 됨 → 테스트 끝에 생성한 id 를 직접 DELETE.

- [ ] **Step 1: 실패 테스트 작성** — `tests/test_llm_weekend.py` 에 추가(파일 상단 import 옆에 datetime/timezone/timedelta/json 보강):
...
def test_reaper_marks_stale_running_failed(db):
    ...
    def status_of(rid):
        with db.cursor() as cur:
            ...
            return cur.fetchone()[0]
    assert status_of(stale) == "failed"      # stale heartbeat → 정리
    assert status_of(nohb_old) == "failed"   # hb 없어도 started 오래됨(disqualify 중 박제) → 정리
    assert status_of(fresh) == "running"     # 최근 heartbeat → 보존
    assert status_of(nohb_new) == "running"  # hb 없고 방금 시작(정상 disqualify 진행 중) → 보존
    assert status_of(current) == "running"   # 현재 실행 → 제외(보존)

    with db.cursor() as cur:                  # cleanup (commit 분 직접 정리)
        cur.execute("DELETE FROM pipeline_runs WHERE id = ANY(%s)",
                    ([stale, fresh, nohb_old, nohb_new, current],))
    db.commit()
```

- [ ] **Step 2: 실패 확인** — `uv run pytest tests/test_llm_weekend.py -k reaper -v` → FAIL (no `reap_stale_weekend_runs`).
...
def reap_stale_weekend_runs(conn, *, current_run_id, stale_seconds: int = 90) -> int:
    ...
    with conn.cursor() as cur:
        ...
        return cur.rowcount
```
(disqualify 머지로 `run_weekend` 이 weekend.run 전에 `disqualify.run` 스윕을 먼저 돌려, 그 구간엔 heartbeat_at 이 아직 NULL. `COALESCE(..., started_at)` 로 그 박제까지 커버. 정상 진행 중인 다른 run 은 started_at 이 최근이라 미정리, 현재 run 은 id 제외.)

- [ ] **Step 4: 통과 확인** — `uv run pytest tests/test_llm_weekend.py -k reaper -v` → PASS.
...
- Test: `tests/test_llm_weekend.py` (기존 weekend 테스트 파일에 **추가** + 기존 깨진 테스트 1개 수리 — Step 6)

- [ ] **Step 1: 실패 테스트 작성** — `tests/test_llm_weekend.py` 에 추가:
...
- [ ] **Step 2: 실패 확인** — `uv run pytest tests/test_llm_weekend.py -k parallel_aggregates -v`
Expected: FAIL — `run()` 에 `concurrency`/`run_id` 인자 없음 + failed_tickers 가 {symbol} 리스트(dict 아님) + 재시도 동작 없음.

- [ ] **Step 3: 구현** — `weekend.py` 상단 import 추가. **주의: `call_claude, ClaudeCLIError` 는 이미 weekend.py 상단에 import 되어 있으므로 재import 금지(중복).** 추가할 것만:
```python
import os, time, subprocess
import psycopg
from concurrent.futures import ThreadPoolExecutor, as_completed
```
워커 함수 추가(`ClaudeCLIError` 는 기존 상단 import 를 사용):
```python
...
- [ ] **Step 4: 통과 확인** — `uv run pytest tests/test_llm_weekend.py -k parallel_aggregates -v` → PASS. (워커가 `psycopg.connect(dsn)` 로 kr_test 에 붙고 `_process_one` 은 mock 이라 실제 DB 작업 없음.)

- [ ] **Step 5: 기존 깨진 테스트 수리(병렬·rs_line·dry_run 정합)** — `tests/test_llm_weekend.py::test_weekend_batch_dry_run_creates_classifications` 는 **현재 baseline 실패**다(원인 2개: ① rs_line 게이트 b2f3188 추가 후 후보 필터가 `rs_line_not_declining_7m=TRUE` 도 요구하는데 셋업이 minervini_pass 만 넣어 후보 0개 → `processed==0`; ② dry_run 의 `_process_one` 은 insert_classification 을 **건너뛰므로** `after-before==3` 단언 자체가 코드와 불일치). 병렬화로 run() 을 건드리는 김에 현실에 맞게 수리한다(= 실제 `_process_one` dry_run 경로의 병렬 end-to-end 검증). 본문을 다음으로 교체:
...
```
(셋업 정리: WK1-3 행은 `db` 픽스처가 같은 conn 에서 commit → 테스트 끝 롤백 대상이 아니므로 잔류하나, ON CONFLICT 멱등이라 재실행 안전. dry_run freeze 쓰기는 워커 conn 커밋 — 기존 sequential 도 동일하게 잔류했음. 결정성은 delta 아닌 절대값이지만 ticker 한정 처리라 무관.)

- [ ] **Step 6: 수리 확인** — `uv run pytest tests/test_llm_weekend.py -v` → 신규 parallel + 수리된 테스트 + reaper(Task2) 모두 PASS.

- [ ] **Step 7: 커밋**
```bash
git add kr_pipeline/llm_runner/weekend.py tests/test_llm_weekend.py
git commit -m "feat(weekend): ThreadPoolExecutor 병렬 + 일시오류 재시도 + failed_tickers 상세 (기존 rs_line/dry_run 테스트 수리 포함)"
```
...
def test_weekend_writes_heartbeat_progress(db, mocker):
    ...
    # 하트비트는 별도 conn 으로 commit → db(READ COMMITTED) 가 최신 details 조회
    with db.cursor() as cur:
    ...
    with db.cursor() as cur:                                  # cleanup
        cur.execute("DELETE FROM pipeline_runs WHERE id=%s", (run_id,))
    db.commit()
```

- [ ] **Step 2: 실패 확인** — `uv run pytest tests/test_llm_weekend.py -k heartbeat -v` → FAIL (run_id 미사용, details 미기록).
...
```
**최종 run() 본문 구조(Task3+Task4 합산, 위→아래 순서):** `concurrency=...` → `processed/failed_tickers/integrity_skipped` 선언 → `dsn=conn.info.dsn` → `workers=max(1,min(concurrency,len(candidates)))` → **reaper(run_id 있을 때)** → `total/prog/prog_lock/stop` → `_heartbeat_loop` 정의 → 초기 heartbeat+`hb_thread.start()` → `try: with ThreadPoolExecutor(max_workers=workers) ...` → `finally: stop.set()+join` → `return {...}`. 즉 Task 3 의 `with ThreadPoolExecutor` 한 줄을 이 reaper~finally 블록으로 대체하며, `workers`·`dsn`·집계변수는 Task 3 선언을 그대로 사용.
...
- [ ] **Step 4: 통과 확인** — `uv run pytest tests/test_llm_weekend.py -v` → PASS (heartbeat + 병렬/재시도 + reaper + 수리된 dry_run 테스트 모두).

- [ ] **Step 5: 커밋**
```bash
git add kr_pipeline/llm_runner/weekend.py kr_pipeline/llm_runner/modes.py kr_pipeline/llm_runner/__main__.py tests/test_llm_weekend.py
...
- [ ] **Step 1: 변경영역 + 인접**
```bash
uv run pytest tests/test_runs.py tests/test_llm_weekend.py tests/test_llm_modes.py tests/test_llm_runner_main.py -v
```
Expected: 신규(run_tracking 2 / parallel / reaper / heartbeat) + 수리된 dry_run 테스트 전부 PASS. modes/main 은 run_id 배선 후에도 기존 통과 유지.

- [ ] **Step 2: 전체 회귀 base 대비**
```bash
uv run pytest tests/ -q 2>&1 | grep "^FAILED" | sed 's/ -.*//' | sort > /tmp/wp_head.txt
wc -l < /tmp/wp_head.txt   # base(merge-base HEAD main) 의 사전 실패 목록(/tmp/wp_base.txt)과 비교
comm -13 /tmp/wp_base.txt /tmp/wp_head.txt   # HEAD 에만 있는 실패(=신규 회귀) → 비어야 함
comm -23 /tmp/wp_base.txt /tmp/wp_head.txt   # base 에만 있는 실패(=수리됨) → test_weekend_batch_dry_run_* 포함 기대
```
Expected: **신규 회귀 0**(`comm -13` 비어 있음). 그리고 `test_weekend_batch_dry_run_creates_classifications`(현재 baseline 실패)가 **수리되어 base 전용 실패로 분류**(이름이 `test_weekend_batch_dry_run_processes_all_qualifying` 으로 바뀌므로 base 의 옛 이름이 사라진 것으로 나타남). (특히 run_tracking 을 쓰는 기존 테스트가 BaseException 확대로 안 깨지는지 확인.)
...
- stale reaper(현재실행 제외, COALESCE(heartbeat_at,started_at) age — disqualify 구간 박제도 커버): Task 2 ✓
...
- 회귀 0 + baseline 깨진 dry_run 테스트 수리: Task 5 / Task 3 Step5 ✓

**2. Placeholder scan:** 모든 코드 스텝에 실제 코드/명령/기대. Task 4 의 in_flight 는 "근사"로 명시(정확 카운트 아님 — 표시용). Task 5 `<base>`=`git merge-base HEAD main`.

**코드 정합 검토(현재 코드 대조 — 2차):**
- 테스트 파일 관례 준수: run_tracking → 기존 `tests/test_runs.py`(추가), weekend(parallel/reaper/heartbeat/수리) → 기존 `tests/test_llm_weekend.py`(추가). 신규 파일 안 만듦.
- `db` 픽스처는 ROLLBACK 격리지만 run_tracking/reaper/heartbeat 는 자체 commit → 새 테스트마다 생성 id 직접 DELETE cleanup(기존 test_runs.py 패턴).
- `call_claude, ClaudeCLIError` 는 weekend.py 상단에 이미 import → Task 3 는 `os/time/subprocess/psycopg/concurrent.futures` 만 추가(중복 import 금지 명시).
- 검증 사실: `_process_one(conn,symbol,market,*,dry_run,as_of)`·dry_run 은 insert 건너뛰고 freeze 만 저장; `get_qualifying_tickers`→`[{symbol,market}]`; `rs_line_not_declining_7m` 컬럼 존재; `call_claude(dry_run=True)`→ _MOCK_GENERATORS stub(외부 의존 0); `finish_run(details=)`·`state["details"]=result`(__main__); pipeline_runs 에 details/started_at/finished_at 존재; disqualify.run 은 run_tracking 자체 래핑 안 함(중첩 없음).

**3. Type consistency:** `_process_one_worker` 반환 dict(status/symbol/attempts/error/detail) ↔ run 집계 일관. `run(concurrency, run_id)` 시그니처 ↔ modes/__main__ 전달 일관. `reap_stale_weekend_runs(conn,*,current_run_id,stale_seconds)` ↔ Task4 호출 일관. heartbeat details 키(weekend_progress/heartbeat_at) ↔ reaper(COALESCE heartbeat_at/started_at)·테스트 일관.

**교차영향(disqualify 머지):** `run_weekend` 이 weekend.run 전에 `disqualify.run` 스윕을 먼저 돌림(modes.py:37-38, 같은 llm_weekend run_tracking 내) → 그 구간 heartbeat_at 은 NULL. reaper 를 `COALESCE(heartbeat_at, started_at)` age 로 두어 그 구간 SIGKILL 박제까지 정리. run_id 배선/하트비트는 weekend.run 한정이라 disqualify 단계엔 무관(단, Task1 SIGTERM 핸들러가 disqualify 포함 전 구간의 graceful kill 을 커버).

**알려진 한계(의도적):** in_flight 는 근사치(표시용). reaper age 기준이 stale_seconds(기본 90초)이므로, 정상 disqualify 스윕이 90초를 넘게 걸리는 동안 **동시에** 또 다른 weekend run 이 시작되면 그 reaper 가 진행 중인 run 을 오정리할 수 있음 — 단 weekend 동시 실행은 실제 시나리오 아님(cron 직렬), 현재 run 은 id 제외로 안전. 웹 표시는 P2.
