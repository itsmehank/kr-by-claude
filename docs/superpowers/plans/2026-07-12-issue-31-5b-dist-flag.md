...
- 5b 쿼리(payload_lite.py:64-79)는 daily_prices 단독, 0-바 제외 조항 보유 — JOIN 추가 시 halt 행은 애초에 리스트에 없어 flag NULL 노출 없음(초기 구간 min_periods NULL 만 가능 → bool|None).
...
| 20d 리스트 0-바 제외 조항 | 불가 | 미미 — halt 행이 리스트에 없어 flag NULL 미노출(초기 구간 NULL 만 bool|None 처리) | EXTENDS | 모니터링(근거: 제외 조항이 선행 필터) |
