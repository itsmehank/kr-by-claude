...
- "진입 부적합"(핸들 깊이 > 12% → faulty → watch)은 A 프롬프트 §4 Gate3 에 **기구현** 확인 → #161 범위에
  추가할 것 없음.
- 갈래 판정(원문 — 전문가 회신 그대로, 수신분):
  - 갈래 2(LLM 보고 % 기반 손절 — `measurements.handle_depth_pct` 로 핸들 저점 역산): **손절가에 LLM
    변동성 유입으로 채택 불가**.
  - 갈래 1·3 의 판정 원문은 본 회신에 별도 문장으로 수신되지 않아, 지시문에 있는 문장만 위에 보존한다
    (갈래 1 = 현행 8% 유지 판정 = 위 첫 항, 갈래 3 = 구조 저점 가격 출력 신설 후 결정론 참조 = 신규 이슈
    로 이관). 원문이 따로 있으면 추가 기록한다.
