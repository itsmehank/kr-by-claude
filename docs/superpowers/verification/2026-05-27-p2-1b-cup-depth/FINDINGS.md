...
## P2-1d (가칭) — wide_and_loose 주간 봉폭 KR vs US 측정 (2026-05-27)

**배경**: triage 에서 wide_and_loose = R 확정 (web). 단 literal 분석상 operative 임계는
"Weekly price swings > 10–15%" = **주간 봉폭 (bar-volatility)**, "1.5–2.5× market" 주석은
non-operative. → 도구는 **P2-1a 일간 σ-ratio 재사용 금지** (분모가 주간 봉폭). 주간 봉폭 직접 측정.

측정: P2-1b 장기 캐시를 주간(W-FRI) 리샘플. 두 metric (literal 민감도):
(i) weekly range % = (high−low)/close, (ii) weekly |close-to-close return| %.
스크립트: `measure_weekly_swings.py`.

### KR median ÷ US median 주간 봉폭 비율 (공통 1996-07~2026-05)
| pair (KR/US) | (i) range | (ii) \|ret\| |
|---|---|---|
| KOSPI / S&P500 | **1.30** | 1.30 |
| KOSDAQ / Nasdaq Comp (성장주 페어) | **1.06** | 1.07 |

(중앙값 %: KOSPI range 3.45 vs S&P 2.66; KOSDAQ 3.64 vs Nasdaq 3.44. 전체역사 비율도 1.22–1.33 로 안정.)

**핵심**:
- **일간 σ 2.3× ≠ 주간 봉폭 1.06–1.30×** — web 경고대로 차원이 다름. 주간 집계가 KR/US 격차를
  크게 줄임 (σ 재사용 시 10–15%→23–35% 로 과대 스케일됐을 것). 두 metric 거의 일치 → robust.
- **성장주 페어 KOSDAQ/Nasdaq = 1.06 ≈ 거의 동일** — base 패턴이 가장 중요한 유니버스에서
  10–15% 가 거의 적정. KOSPI(대형·broad)만 1.30.

### 스케일 권고 (web 책-충실 판정 입력)
| pair | 비율 | 10–15% × 비율 |
|---|---|---|
| KOSPI/S&P500 | 1.30 | ~13–19% |
| KOSDAQ/Nasdaq | 1.06 | ~11–16% |

비율≈1 인 KOSDAQ 는 유지에 가깝고, KOSPI 는 modest 상향(13–19%). clamp 은 web 판정
(예: floor=현행 10% 유지). **판정은 web 세션.**

### [3] 주석/동작 불일치 — 설계 결정 사항 (기록)
`analyze_chart_v3.md:189` 괄호 "1.5–2.5× general market correction" 은 **operative 아님**
(발동은 10–15% 주간 봉폭). 보정 시 둘 중 하나를 web 이 택해야 함:
- **(①) 주석을 동작에 맞춤**: wide_and_loose 를 순수 bar-volatility flag 로 명확화하고
  "1.5–2.5× market" 주석 제거/수정 (size-relative 는 cup depth=P2-1b 가 이미 담당).
- **(②) size-relative 를 실제 추가**: 봉폭(bar-width)과 base 깊이(size-relative)를 2차원으로
  분리해 각각 flag — 단 깊이 차원은 P2-1b 가 "유지" 판정한 영역이라 중복 위험.
→ ① 이 단순·정합 (깊이는 P2-1b/cup, 봉폭은 P2-1d/wide_and_loose 로 책임 분리). web 결정.

---

## 재현
```
uv run python docs/superpowers/verification/2026-05-27-p2-1b-cup-depth/measure_drawdowns.py      # P2-1b cup depth
uv run python docs/superpowers/verification/2026-05-27-p2-1b-cup-depth/measure_weekly_swings.py  # P2-1d 주간 봉폭
```
