...
- 3개 프롬프트(`analyze_chart_v3.md`, `evaluate_pivot_trigger_v1.md`, `calculate_entry_params_v2_0.md`)에 **"제공되는 모든 가격(OHLCV·차트·지표)은 수정주가(split-adjusted) 기준" 명시 한 줄** 추가. `analyze_chart_v3.md:60` 의 기존 adjusted 언급은 전환 후 **사실이 됨**(문구 검증·유지). 출력 스키마는 변경하지 않음(entry_price 계약은 비목표).
...
- 변경: `payload_builder.py`(SELECT 2), `chart_render.py`(plot 컬럼 2 함수), `payload_lite.py`(build_for_5b/6), `failed_breakout.py`(close), `handle_quality.py`(4컬럼), 프롬프트 3개(명시 한 줄).
