...
### 10.2 통합 — `tests/test_market_context_status.py` (확장)

**End-to-end**:
- 가짜 KOSPI / KOSDAQ index_daily fixture (각 σ 다르게)
- status.py 호출 → 두 시장이 *다른* 보정 임계로 detect_last_ftd / count_distribution_days 호출됨 확인
...
| `kr_pipeline/market_context/compute/status.py` | Data flow (Section 6) 구현 — 시장별 σ 측정 → 보정 임계 → follow_through / distribution_day 호출 시 전달 |
| `tests/test_market_context_status.py` | 통합 테스트 확장 (Section 10.2) |
