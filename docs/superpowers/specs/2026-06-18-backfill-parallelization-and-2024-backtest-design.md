...
추출이 건드리는 모든 경로를 실제 코드로 확인한 결과:

**1) 테스트 mock 호환성 (가장 중요 — 깨지기 쉬움).**
- weekend 병렬 테스트 대부분이 `mocker.patch.object(wk, "_process_one", ...)`로 모킹하고,
  `test_weekend_worker_connect_failure`는 `mocker.patch.object(wk.psycopg, "connect", ...)`로 모킹.
- 따라서 **워커는 `call_claude`를 직접 부르지 않고 `process_fn`(=각 모드의 `_process_one`)만 호출**해야 함.
  `_process_one`은 각 모드 모듈에 그대로 남으며(weekend는 freeze 저장 포함, backfill은 미저장),
  거기서 `call_claude`/`build_analysis_inline`/`save_freeze`를 부름 → 기존 mock 전부 유효.
- weekend.run/backfill.run은 `run_parallel_batch(..., process_fn=_process_one)`를 **호출 시점에 모듈 전역**
  으로 전달 → `wk._process_one`/`bf._process_one` 패치가 그대로 적용됨.
- `wk.psycopg.connect` 패치는 psycopg **모듈 전역** 속성을 바꾸는 것이라, 워커가 `parallel.py`로
  옮겨가도 같은 psycopg 객체를 공유하므로 적용됨. **단 이 가정은 깨지기 쉬움** → connect-failure
  테스트를 추출 직후 반드시 재실행. 깨지면 폴백: 워커가 connect 함수를 주입받거나, 테스트를
  `parallel.psycopg.connect` 패치로 갱신.
- weekend 내부 심볼(`_process_one_worker`/`_write_heartbeat`/`reap_stale_weekend_runs`)을 **외부에서
  직접 import/참조하는 곳 없음**(grep 확인) → 이동 안전.

**2) backfill.run 반환 dict 모양 보존.** `__main__.py:119-130`이 `result.get("processed")`(rows_affected),
`state["details"]=result`로 소비. 현재 backfill 반환 키 = `weeks, processed, skipped_existing,
failures, failed([symbol,str(as_of),err]), start, end`. → 공용 헬퍼는 `failed_tickers`(dict 리스트)를
반환하므로, backfill.run이 **현재 as_of로 `[symbol, str(as_of), error]` 형태로 매핑**해 기존 키를 유지.
`integrity_skipped`는 backfill에 신규(additive 키로 추가, `failures`에는 미포함 — weekend와 동일 의미).

**3) backfill 동작 변화(의도된 개선, 명시 필요).** 공용 워커 경유로 backfill에 다음이 *추가*됨:
- transient(`TimeoutExpired`) 워커 재시도 ×2 (현재 backfill은 재시도 없음)
- `DataIntegrityError` → skip 처리(현재는 generic except로 failure 집계)
- 워커 connect 실패 → 그 종목만 실패(배치 중단 안 함)
→ 모두 weekend와 동일해지는 개선이나 *동작 변화*이므로, 기존 backfill 테스트
(`test_backfill_aborts_on_usage_limit`, `test_backfill_resume_after_usage_limit`)를 병렬 의미에 맞게
검토·갱신(결정성 위해 `concurrency=1` 고정 등).

**4) abort 수명주기(다중 토요일).** backfill.run이 **단일 `abort` Event**를 만들어 매 토요일 헬퍼
호출에 전달. 어느 토요일이든 `usage_limited`면 **토요일 루프 break + `UsageLimitError` 전파**
(현재 즉시중단+resume 동작 보존). abort set 이후 토요일의 워커는 즉시 aborted 반환.

**5) heartbeat — backfill은 의도적으로 OFF(비범위, 근거 명시).** `__main__.py:91,112`상 backfill도
`run_tracking`으로 감싸져 `run_id`가 **이미 존재**하나 backfill.run에 미전달. 이번 작업에선 heartbeat=None
유지. 근거: 헬퍼 `prog`는 호출 단위(=토요일 1개)라, 토요일을 가로지르는 전역 진행률은 별도 집계가
필요(YAGNI). 관측은 INFO 로그 + resume으로 충분. run_id가 있으므로 후속 확장 용이.
부수: `llm_backfill`용 reaper는 없음(weekend 전용) → kill -9 시 stale 'running' 잔존은 **현재와 동일**(회귀 아님).

**6) main 커넥션 트랜잭션 위생.** `connect()`는 autocommit=False(READ COMMITTED). 다중 토요일 루프에서
main 커넥션의 `_already_backfilled`는 워커들의 commit을 매 statement 재조회로 **본다**(resume 정합성 OK).
단 한 트랜잭션을 실행 내내 길게 잡지 않도록, **토요일마다 read 단계 후 main 커넥션 commit/rollback**으로
스냅샷 해제(현 순차코드는 종목별 commit으로 이미 해제하던 위생을 유지).

**7) CLI `--concurrency` 신규.** `__main__.py`에 현재 없음 → `--concurrency`(int) 추가하고 backfill.run에
전달(+`BACKFILL_CONCURRENCY` env 폴백, 기본 4). **weekend 경로는 미변경**(기존 `WEEKEND_CONCURRENCY` 유지).

**8) DB 커넥션 수.** 토요일당 동시 ≤concurrency개 워커 커넥션(+main 1). 토요일 간에는 순차라 동시
최대 ≈ concurrency+1. localhost 기본 한도 대비 무시 가능.

**9) 무관 확인.** `assert_data_fresh`는 backfill 제외(`__main__.py:94`), `modes.run_weekend`은
weekend.run 시그니처/반환만 유지하면 영향 없음(`modes.py:40`), `--tickers`/`limit` 의미 불변
(tickers=게이트 내 필터, limit=토요일별 적용).
...
2. `parallel.py`로 공용 함수 추출(워커 + 실행루프 + 선택적 heartbeat), weekend.run이
   그것을 호출하도록 교체 — **동작 변화 0이 목표**. 위 11개 테스트가 그대로 green인지 확인.
   특히 `test_weekend_worker_connect_failure`(psycopg.connect 전역 패치 가정)와
   `test_weekend_writes_heartbeat_progress`(heartbeat가 헬퍼로 이동)를 우선 확인.
3. backfill.run을 공용 함수 사용으로 전환: 단일 abort, 토요일별 `_already_backfilled` skip 유지,
   반환 dict 기존 키 보존(`failed_tickers`→`failed` 매핑, `integrity_skipped` additive),
   토요일별 main 커넥션 commit/rollback, `usage_limited`면 루프 break + raise.
4. `__main__.py`에 `--concurrency` 추가 + backfill.run 전달(`BACKFILL_CONCURRENCY` 폴백).
5. backfill 병렬 테스트 추가/갱신(weekend 테스트 본떠, 결정성 위해 `concurrency` 명시):
   병렬 집계+transient 재시도, connect 실패 흡수, 사용량 한도 abort(기존 테스트 병렬판),
   `DataIntegrityError` skip, resume skip 유지, interrupt 취소. 반환 dict 키 보존 assert.
6. 전체 `uv run pytest tests/` 회귀 판정(base↔HEAD 실패 수 비교 — 메모리상 baseline ~31개).
...
```bash
# 1) dry-run 사전 점검 (LLM 비용 0이나 차트 PNG 빌드는 실제 수행 — 입력 빌드 경로 검증):
#    8종목이 각 토요일에 후보로 잡히고 inline+차트 빌드가 에러 없이 되는지
