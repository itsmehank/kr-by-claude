...
## 8. 측정 결과 (2026-07-21 실행 — append, 본문 §1~7 무수정)

실행: `scripts/stage3_replay_climax_topping.py`, 원자료
`data/verification/2026-07-21-stage3/replay_results.json`. LLM 호출 0회, production
DB 는 SELECT 만(쓰기 없음). `--limit 100` 스모크 통과 후 전 모집단 실행.

### 사전등록 지표 (문언 그대로)

| 지표 | 값 |
|---|---|
| 모집단 | 4,252건(rows_total) |
| 재생 성공 | 4,252건(rows_replayed_ok) — build_errors 0건 |
| baseline 분포(⑥) | anchored 431(10.1%) / no_transition 3,821(89.9%) / left_censored 0(0%) |
| quality_flag_climax | False 4,252(100%) — 이 재생 모집단에서 0건 발화 |
| quality_flag_topping | False 4,252(100%) — 이 재생 모집단에서 0건 발화 |
| **① §6.1 후보율** | 68/4,252 = **1.60%** |
| ① 후보 건 기존 classification | watch 38 / ignore 30 / entry 0 |
| **② branch_B**(G0∧TB) | 0/4,252 = **0.00%** |
| **② branch_D**(G0∧TD분배일) | 38/4,252 = **0.89%** |
| ② branch_either | 38/4,252 = **0.89%**(= branch_D 와 동일, B 가 0 이므로) |
| ③ branch_either 기존 classification | watch 16 / ignore 22 / entry 0 |
| **③ ignore 정합률** | 22/38 = **57.89%** |
| would_force(강제율, gates.py 조건 재현) | 16/4,252 = **0.38%** |
| **④ anchor 재실행 불일치** | 0/500(표본, seed=44) = **0건** |
| ④ anchor 연령 분포(주, anchored 431건) | min 0 / p25 21 / p50 73 / p75 160 / max 245 |
| **⑤ T4 max(7~15) 발화율** | 1,229/4,252(non-null 4,252) = **28.90%** |
| **⑤ T4 고정10일 발화율** | 739/4,252(non-null 4,252) = **17.38%** |
| ⑤ 발화율 차이 | **11.52%p** |

### 판독 (§4 밴드 적용)

- **§6.1 후보율 1.60% ≤ 30%** → 과발화 의심 **미해당**. E1 미고려 상한이라는 한계(§5)를
  감안해도 후보 자체가 희소해 §6.1 게이트가 과발화 방향으로 작동할 우려는 낮다.
- **§6.2 ignore 정합률 57.89%** → **30~60% 밴드 = "규약 재검토 신호"**(60% 문턱에 근접하나
  미달). 판정: **정합(통과) 아님** — 셋 중 중간 밴드. §6.2 shadow 가 fire 하는 38건 중
  22건(57.9%)은 이미 LLM 이 ignore 로 판정해 게이트와 방향이 일치하지만, 나머지 16건
  (42.1%, watch 유지 중)은 게이트가 추가로 포착하는 몫이다 — 표본이 38건으로 작아
  (n<40) 이 비율의 신뢰구간이 넓다는 점도 함께 기록. **결론: §6.2 shadow 활성화
  전 표본 확대(더 긴 기간/더 많은 종목 재생 또는 shadow 런타임 관측 축적)를 권고**하되,
  이 규약 자체를 폐기할 근거(<30%)는 아니다 — D3 의 "shadow 로그 유지, 활성화는 별도
  결정" 을 그대로 지지.
- **would_force 0.38% ≪ 5% 상한** → 강제율 경보 **미해당**. §6.2 shadow 를 활성화해도
  예상 강제 강등 비율은 등록된 상한 대비 여유가 크다(활성화 여부는 이 계획 범위 밖 — 유예 유지).
- **anchor 재실행 불일치 0건** → **결정론 확인, 실패 없음**(밴드: 0건=성공).
  anchor 연령은 0~245주로 넓게 분포(p50=73주) — anchor 가 최근 주부터 먼 과거까지
  고르게 걸쳐 있어 "가장 최근 전환" 탐색이 특정 구간에 쏠려 있지 않음을 보여준다.
- **T4 확정 규칙**: max 28.90% vs 고정10일 17.38%, 차이 **11.52%p ≥ 2%p** →
  사전등록 §4 규칙대로 **두 수치를 보고하고 확정은 사용자 결정으로 이관**한다.
  **판정: "사용자 결정 대기"**. 참고로 max variant 가 고정10일보다 발화가 11.5%p
  더 관대(자유도가 높아 7~15 중 최적 창을 항상 고를 수 있으므로 구조적으로 max ≥
  고정10일 — 방향은 항상 max 쪽이 더 잦게 발화함).
- **③ 정합률(§6.1 후보 대비)**: 참고 수치로 병기 — 68건 중 watch 38 / ignore 30(44.1%가
  이미 ignore). §6.1 에는 사전등록된 밴드가 없어(§4 는 §6.2 에만 정합률 밴드 등록)
  판정 대상 아님, 기록만.

### 진단 노트 (사전등록 외 — 라벨 명시, 판독에 영향 없음)

- **branch_B(G0∧TB) = 0건**의 원인을 독립 진단했다: `tb_ok`(10주선 아래 **연속** 8주
  이상)는 정의상 `g0_below_10w`(마지막 주가 10주선 아래)가 성립해야만 카운트가
  시작되는데(마지막 주가 위에 있으면 즉시 0으로 중단), 이 재생 모집단에서
  `g0_below_10w=True` 인 153건 중 `tb_weeks_below_10w` 최댓값이 **7주**(8주 도달 0건,
  분포: 1주 99 / 2주 28 / 3주 12 / 4주 6 / 5주 5 / 6주 2 / **7주 1**)였다.
  `backtest_classification` 모집단이 이미 미너비니 트렌드템플릿 스크리너를 통과한
  entry/watch/ignore 후보(스크리너가 상류에서 장기 하락 종목을 걸러냄)라, 10주선
  아래 8주 이상 머무는 사례가 구조적으로 희소한 것으로 판단된다 — 게이트 로직
  결함이 아니라 **모집단 특성**(코드는 fixture 단위테스트에서 8주 발화를 이미 검증함,
  `tests/test_climax_topping.py::test_topping_g0_tb_fire`).
- `quality_flag_climax`/`quality_flag_topping` 이 이 재생 모집단에서 0건 발화한 것은
  `_fetch_weekly_full` 의 zero-bar 제외 규약(:295-317)이 이미 0/None 종가를 걸러내기
  때문으로 보이며, 이슈 #49(adj 품질)의 영향이 이 게이트 산술 표면에서는 검출되지
  않았다는 뜻이지 #49 자체가 해소됐다는 뜻은 아니다(§5 한계 그대로 유효 — 이 재생은
  #49 검증 범위가 아니다).

### D4/D3/D5① 결론

- **D4(검증)**: 재생 4,252/4,252 성공, anchor 결정론 500/500 확인, 밴드 적용 결과
  §6.1 과발화 없음·§6.2 정합 중간 밴드(재검토 신호)·강제율 여유 큼 — **소급 재분류
  없이** 종료.
- **D3(강제율 상한)**: 0.38% ≪ 5% — 등록된 상한 충족, 활성화 결정은 여전히 이 계획
  범위 밖으로 유예.
- **D5①(T4 창 규약)**: **미확정 — 사용자 결정 대기**(차이 11.52%p ≥ 2%p 문턱).
  두 수치를 그대로 보고했으니 다음 세션에서 사용자가 max/고정10일 중 확정하면
  `climax_topping.py` 의 T4 주석("잠정")을 확정 문언으로 갱신하고 Task 9 프롬프트
  개정에 반영해야 한다.
- **제3안(watch_reason) 카운터**: §7 등록 그대로 — Task 9 배포 전이라 측정 대상
  자체가 존재하지 않음을 재확인(0건, 예상대로).

## 결정 기록 (2026-07-21 사용자 확정 — append)

- **T4 창 규약 = any-window(max) 확정** (§8 이관 항목의 사용자 결정): 외부 자문(책 근거
  검토)도 any-window 판정 — 출처 정정 포함: 70%/7~15일 공식은 TTLC Ch.9, HMMS 는
  "7 of 8 or 8 of 10" 별도 서술. 상승일 정의(close[t]>close[t-1], 보합 비상승·분모
  포함)·끝점 고정·비율 엄격 적용은 구현과 문자 일치 확인(코드 변경 없음). 태그:
  70%·7~15 book-mandated(TTLC) / any-window 기계화 design-judgment.
- **§6.2 정합률 57.89% 처리 = 불일치 16건 판독 분석 실행**(사용자 선택 b) — 결과는
  별도 문서로 append 예정.
