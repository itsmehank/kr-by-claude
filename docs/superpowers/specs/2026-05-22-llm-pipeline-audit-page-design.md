...
- 새 코드 수정. spec 작성 과정에서 발견된 비일관성은 audit 페이지 구현 이전에 fix 완료 (§9.1 변경 이력 참조). 본 페이지는 fix 후 정확한 시스템 상태만 서술.
...
- **기존 안내 페이지 mermaid 도 동일하게 갱신됨** (2026-05-22 a215cfa): `DIAGRAM_DATA_FLOW` 에 weekend 노드 추가 + `trigger_type='breakout'` 안전장치 반영. `DIAGRAM_STATE` 의 잘못된 promotion + go_now 전이 제거. audit 페이지와 안내 페이지가 일관된 흐름 표시.
...
**파일**: `prompts/evaluate_pivot_trigger_v1.md` (127 행, 2026-05-22 SMA-21 가드 + days_since_classification 추가 후)
...
| 10 | `market_index_daily.csv` | **종목 시장의 인덱스** 일봉 (KOSPI=1001 또는 KOSDAQ=2001) | csv_builder.py (lookback=60) |
| 11 | `market_index_weekly.csv` | 같은 인덱스 주봉 | csv_builder.py (lookback=104) |
...
`zip_builder.py:75`: `index_code = INDEX_CODE_MAP.get(market, "1001")` — 종목 market 에 따라 KOSPI(1001) 또는 KOSDAQ(2001) 의 인덱스 사용. 파일명 `market_index_*` 로 시장 중립적 표기 (2026-05-22 a215cfa 이전 `kospi_*` 명칭은 KOSDAQ 종목 분석 시 혼동 유발하여 일반화).
...
<details>
  <summary>2. evaluate_pivot_trigger_v1.md (evaluate_pivot, 127 행)</summary>
...
##### E. spec audit Part 1-7 검토 후 코드 정합성 fix (2026-05-22, commit `a215cfa`)

- 사유: spec v2 작성 과정의 line-by-line 비교에서 발견된 코드 정합성 이슈 사전 정리.
- 변경 1 — `daily_delta` SQL 에 `delisted_at IS NULL` 필터 추가 (compute/delta.py). weekend 의 `get_qualifying_tickers` 와 일관성 회복. 상장폐지 종목이 daily_delta 에 잡힐 잠재 버그 차단.
- 변경 2 — ZIP payload 의 `kospi_*.csv` → `market_index_*.csv` 일반화 (zip_builder.py + README + 테스트). INDEX_CODE_MAP 이 종목 시장별로 KOSPI/KOSDAQ 인덱스 선택하는데 파일명이 "kospi_*" 로 고정되어 있던 혼동 해소.
- 변경 3 — `LlmPipelinePage` mermaid 다이어그램 정정. `DIAGRAM_DATA_FLOW` 에 weekend 노드 추가 + `trigger_type='breakout'` 안전장치 반영. `DIAGRAM_STATE` 의 `Watch --> EntryParams: promotion + go_now` 잘못된 전이 제거 + weekend batch 재분석 경유 흐름 명시.

##### F. 전문가 자문 #2 + #3 반영 (2026-05-22, commit `0e0976c`)

- 사유: spec audit 작성 후 잔존 3 항목 (#1 retry / #2 c6 임계 / #3 SMA20) 전문가 자문 요청 → 답변 받음.
- 변경 1 — `minervini.py:38` c6 임계 주석 보강. *TTLC* Ch.6 +25% (최신작, 현재 코드 일치) vs *TLSMW* Ch.5 +30% 두 저작 간 버전 차이 명시.
- 변경 2 — SMA-21 (≈ 20-day line) 가드를 LLM prompt 에 추가 (옵션 2 채택, 게이트는 SMA-50 그대로 유지):
  - `payload_lite.py`: `current_metrics.sma_21` + `prior_analysis.days_since_classification` 추가
  - `evaluate_pivot_trigger_v1.md` §3.1 breakout abort + §3.2 invalidation abort 에 20일선 가드 + "단독은 wait" 단서 (책: 단독 무의미) + squat reversal recovery 여지 (Minervini *TLSMW* Ch.10)
- 변경 3 — #1 retry 정책은 책 밖 (엔지니어링). 현행 합리적 (전문가 결론). 코드 변경 없음.
...
##### 코드 정합성 이슈 (모두 해결 — 2026-05-22 a215cfa, §9.1.E 참조)
- **daily_delta SQL `delisted_at` 필터 누락 — 해결**: `compute/delta.py` 에 `JOIN stocks WHERE s.delisted_at IS NULL` 추가 완료.
...
- **`kospi_daily/weekly.csv` 파일명 혼동 — 해결**: `market_index_daily.csv` / `market_index_weekly.csv` 로 일반화 완료.

##### 기존 안내 페이지 다이어그램 정정 — 해결 (2026-05-22 a215cfa, §9.1.E 참조)
- `/docs/llm-pipeline` (안내 페이지) 의 `DIAGRAM_DATA_FLOW`: weekend 노드 추가 + `trigger_type='breakout'` 안전장치 반영 완료.
- `DIAGRAM_STATE`: `Watch --> EntryParams: promotion + go_now` 잘못된 전이 제거 + weekend batch 재분석 경유 흐름 추가 완료.
...
- §7 ZIP 13 파일 + README 본문 (market_index_* 명칭 일반화 반영).
- §8 3 prompt 전체 (각 309/127/580 행) details 접기.
