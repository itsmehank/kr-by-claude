...
4. trigger_type 실분포: promotion 107 / invalidation 21 / breakout_from_watch 13 /
   ...
   promotion 만 반복될 수 있다**(fresh_cross·거래량 배수·watch_reason 요건 미충족 시).
...
6. 거래정지일에도 `daily_prices` 행이 존재하며 **adj_close 는 직전가 carry 로 유지**
   된다(`nullify_halt_adj` — null 이 되는 것은 adj OHLV·거래량). halt 판정 관례 =
...
- 시간창(as-of) 재구성은 쓰지 않는다 — 주말 배치가 새벽 실행되며 `analyzed_for_date`
  를 소급(backdate)하는 케이스에서 ground truth 와 **26/142(18%) 불일치**가 실증됐다.
...
- 구간 끝 t' = 같은 symbol 의 **다음 classification 행**(source 무관 — disqualify 포함)의
  `(key_date, classified_at)` 사전식 비교 기준 직후 행의 key_date. 없으면 오늘.
- 이 구간은 **성과·스파크라인 창 계산에만** 쓴다(귀속은 위 직접 조인).
  구간을 넘겨 연장하지 않는다 — 재분석(새 pivot) 이후 가격을 옛 pivot 대비로
  계상하면 인접 회고 행과 이중 계상되기 때문.
...
- **staging** = promotion 만 발생(돌파 계열 없음). §0-4 실측대로 pivot 을 이미
  ...
  **"staging(미확정)"** 으로 쓰고, 최고 도달률이 0% 이상(pivot 상회)인 staging 행은
  화면에서 구분 표기한다.
- 상태 축: 미발동 / staging / 돌파-진행중 / 돌파-완료. invalidation 은 타임라인에만
  표시(하향 사건). breakout_from_watch 는 promotion 선행 없이도 발생한다(13건 중 9건).
...
- **미발동·staging 분석**: T+5/T+20 대신 **최고 도달률** =
  `max(adj_close) / pivot − 1`, 창 = **성과 구간 내**(`[key_date, t')`, 연장 없음).
  이 값은 스냅샷 체인이 불가하므로(트리거 없음) 창 내 기업행위 발생 시
  **왜곡 가능 배지**(`corp_action_flag`)를 표시한다. 배지 판정 =
  `corporate_actions.event_date ∈ [key_date, 성과 창 끝]`, 분석 행 단위 boolean.
  (실효 규모 참고: 기본 필터(pivot 있음) 기준 현재 노출 1건.)
...
- **돌파 행**: 앵커 = 돌파일 D, 범위 = D ~ min(D+20거래일, 오늘) — T+20 시점이
  항상 차트 안에 있다.
- **미발동·staging 행**: 범위 = 성과 구간 `[key_date, t')` (최고 도달 지점이 항상
  차트 안). 구간이 20거래일보다 길면 그대로 다 그린다(점 수 상한 60 — 초과 시
  균등 다운샘플).
- pivot 기준선 표시. API 응답에 숫자 배열 인라인.
...
- **신규 라우터** `api/routers/review.py`: `GET /api/review/analyses`
  ...
  - 응답 메타: `orphan_trigger_count` (기간 내 귀속 불가 트리거 수).
...
7. 미발동 최고 도달률: 창 = 구간 내 한정(연장 없음), 0길이 구간 → "—",
   corp_action_flag 판정.
8. API 계약: 필터 조합(triggered·include_pivot_null)·limit 상한·빈 결과.
9. 기존 suite 무회귀 (실패 0 기준 유지).
