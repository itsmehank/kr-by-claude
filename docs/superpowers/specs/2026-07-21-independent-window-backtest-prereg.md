...
**Arm-54 — 고정(2026-08-14, 사용자 게이트 승인).** 정의 원문 =
`docs/superpowers/specs/2026-08-14-issue54-pilot-path.md` (LOCKED, 본 커밋 동봉). 요약:

- **Arm-54 = Arm-53 사다리 + 파일럿 예외 경로**: 발동 = rally_attempt ∧ dist<6,
  자격 = 3중 하드 필터(`minervini_pass` ∧ `rs_line_at_52w_high` ∧
  `pct_from_52w_high` ∈ [−15, −5]), 사이징 = 정상의 50%(`pilot_frac=0.5`) ×
  기존 flag 배수(#80 동결 불변) · 파일럿 스톱 6% · 에피소드 캡 2,
  전역 연속 손절 3회 잠금(해제 = 새 FTD 단독). 트리거 평가(#45 추격 상한 포함)
  그대로 상속. LLM 무관여(전부 결정론 선계산).
- **판정 = 2단(F-S2 선례)**: ① 관찰(look) — 표본 A+B(2021~24, 비보호)에서
  Arm-53 vs Arm-53+54, 통과 기준 = 파일럿 한정 mean excess_net>0 AND
  final_multiple 비악화. 미통과 시 C 개봉 없이 기각. ② 재현 — 표본 C 에서 관찰
  확정 방향 재현 AND MDD ≤5pp(§4 문언 원형). C 개봉은 두 arm 판정 겸용 1회.
- 파일럿 전용 산출물: 진입 수·승률·평균 손익, 잠금 발동·놓친 진입, 캡 도달 분포,
  필터별 단독 탈락 기여도, floor(3.0%) 도달 빈도, dist<6 배제 후보 수.
