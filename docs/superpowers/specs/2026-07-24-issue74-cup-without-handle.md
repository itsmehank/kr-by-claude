...
## 7. 부록 — F1~F3 제재의 재매핑 (2026-09-08, #153; §6 원문 불변)

§6 사전등록 원문의 "F1~F3 중 어느 하나 발화 → 장치 강화(사이징 ×0.5 등) 1회 시도"에서
**사이징 ×0.5 는 "해당 코호트(cup_without_handle) 의 R × 0.5 = 0.625%"** 로 읽는다.
근거: production 사이징이 리스크 역산으로 교체되어(#153) 배수 감액 구조가 소멸했고, Minervini
TTLC Ch.3 "실적 악화 시 리스크(R)를 조인다" 가 같은 처방이다 [design-judgment]. 재발화 시 B 회귀
불변. F4 무관. 코호트 판독 자체(#59/#45 프레임, go_now ≥ 20)는 불변. `no_handle_shakeout_absent`
flag 주입·strict 1.5× 거래량 표기는 유지된다(진입 게이트 역할 — 사이징 감액 4.9pp 만 소멸).
