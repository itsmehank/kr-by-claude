...
- 구간 메타: `시작일`, `마지막 분석일`, `분석 주 수 N`(실제 행 수만 — 갭 주를
  세지 않음. 백테스트 해석 왜곡 방지).
- **기간 창 잘림 처리(필수)**: 조회 창이 차트 기간에 잘리므로, **첫(가장 오래된)
  구간의 시작일은 전환일이라 단정할 수 없다** — 창 이전에 같은 분류가 이어졌는지
  데이터 밖이라 판별 불가. 첫 구간은 항상 시작일을 **"기간 이전부터 ~"** 로
  보수 표기하고, 대표 사유도 "(기간 내 첫 기록 기준)" 캡션을 붙인다.
  나머지 구간의 시작일만 "전환일"로 표기.
- **소스별 구간 모양 차이(명시)**: 라이브는 탈락 시 disqualify 스윕이
  disqualified 행을 끼워 구간이 분할되지만, **백필은 스윕이 없어 미통과 주가
  갭으로만 남아** 같은 분류가 한 구간으로 합쳐진다(예: watch 5주 → 미통과 3주
  갭 → watch 4주 = "watch · 9주 분석" 한 구간). 갭의 존재는 기간 표기와 차트
  밴드로 드러난다 — 버그가 아닌 의도된 단순화.
...
- **프론트(vitest)**: `groupHistorySegments` — 빈 입력 / 단일 구간 / 분류 교차
  ...
  **첫 구간 창-잘림 플래그**(가장 오래된 구간에 `truncatedStart` 마킹).
- tsc + 프로덕션 빌드.
