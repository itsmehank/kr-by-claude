...
- **stale reaper** `reap_stale_weekend_runs(conn, *, current_run_id, stale_seconds=90)`: weekend 시작 시, `pipeline='llm_weekend' AND status='running' AND id <> current_run_id AND details->>'heartbeat_at' IS NOT NULL AND (details->>'heartbeat_at')::timestamptz < now()-interval` 인 행을 `status='failed'`, error="stale heartbeat — process likely killed" 로 UPDATE. → `kill -9`/크래시 박제 자동 정리. **현재 실행(run_id)은 제외, heartbeat 없는 행은 미매칭**(오정리 방지). (현재 실행은 §위 초기 heartbeat 로 곧 heartbeat_at 을 가지므로, 다음 실행의 reaper 가 정상 추적.)
