...
- **워커**: 자기 커넥션 `psycopg.connect(conn.info.dsn)` 열고(메인 커넥션 dsn 재사용 — **검증: kr_test dsn 재연결 OK**, 테스트/프로덕션 동일 DB dsn — passwordless localhost 라 재구성 가능) → `_process_one` 로직(build_analysis_zip + call_claude + insert_classification + save_freeze) + commit + close.
...
- **하트비트 스레드**(자기 커넥션, run_id 있을 때만): **시작 즉시 1회 기록(t=0)** 후 30초마다 + (메인 루프가 완료마다 갱신하는) 공유 진행상태(lock 보호: done/total/in_flight/failed)를 읽어 `UPDATE pipeline_runs SET details = <merge {weekend_progress:{...}, heartbeat_at:<utcnow iso>}> WHERE id=run_id` + commit + 로그("weekend: 37/120 done, in-flight 4, failed 2"). **초기 즉시 기록 이유**: 첫 30초 틱 전에 kill 돼도 heartbeat_at 이 존재해 90초 후 stale 로 reaper 가 잡음(없으면 박제). weekend.run 종료 시 스레드 stop+join(이후 run_tracking 이 최종 details=전체 result 기록).
- **stale reaper** `reap_stale_weekend_runs(conn, *, current_run_id, stale_seconds=90)`: weekend 시작 시, `pipeline='llm_weekend' AND status='running' AND id <> current_run_id AND COALESCE((details->>'heartbeat_at')::timestamptz, started_at) < now()-interval` 인 행을 `status='failed'`, error="stale heartbeat — process likely killed" 로 UPDATE. → `kill -9`/크래시 박제 자동 정리. **현재 실행(run_id)은 제외**(오정리 방지). age 기준이 `COALESCE(heartbeat_at, started_at)` 이라 **heartbeat 가 아직 없는 박제**(예: disqualify 스윕 구간에서 SIGKILL)도 started_at 이 오래되면 정리. (disqualify 머지로 `run_weekend` 이 weekend.run 전에 `disqualify.run` 을 먼저 돌려 그 구간 heartbeat_at 이 NULL 인 점을 커버.)
