...
이유: "이건 임계 변경 아닌데?" 오판 방지. 세션/외부 문서의 방법론 판정은 이 사실 트리거를 면제하지 못한다(governance.md 원칙 2 — #151 사례). P2-1a 도 "FTD 임계 하나 바꾼 줄 알았는데 파생 신호 (last_ftd_date) 를 건드려 룰 3 까지 영향" — 주관 분류로는 안 걸렸을 케이스. "thresholds.py 또는 그 소비처를 건드렸나" 라는 *사실* 로 걸린다.
