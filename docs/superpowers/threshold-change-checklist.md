...
- 2026-07-12: #29 B §3.5 형제 분기(marginal_tt·valid_base)에 flag 조건부 시장 재확인 추가 (dist≥5 역류 옆문 폐쇄). 의존성 맵 = docs/superpowers/plans/2026-07-12-issue-29-sibling-market-recheck.md. 임계 리터럴 사본 미증가(unfavorable_market 게이트 참조 방식) — 사본 금지는 tests/test_prompt_trigger_gates.py 가 강제. 동반: A측 임계 추출 §3.5 스코핑 + 산문 리터럴 정리(3→1사본). 방향 = 보수화만(AND). C §7 면제의 전제('B 통과=시장 회복')가 전 경로에서 성립하게 됨. ⚠ D4 공백의 거울 방향(사유=unfavorable 기록 시 marginal clean-TT 재확인 생략)은 flag 부재로 본 방식으론 폐쇄 불가 — #22 재설계 검토 대상(PR #34 리뷰 발견).
- 2026-07-12: #31 5b 분배일 판정을 LLM 재계산 → distribution_day_flag 전달+authoritative 규약으로 재배선(이중 정의 해소). 의존성 맵 = docs/superpowers/plans/2026-07-12-issue-31-5b-dist-flag.md. STOCK_DISTRIBUTION_PCT_DOWN 이 이제 B 게이트 발동도 직접 구동 — #30 의 B-수치 재검토에 'B 게이트 발동률' 연동 명시. 3일/5일/3개 창·개수는 프롬프트 전용값 비등재 유지. 구 계획 문서(2026-05-22 :55) superseded 추기.
...
- 2026-09-08: #166 이익목표 절반매도(5B) production 이식(플래그 OFF, 파리티). +SELL_HALF_ENABLED=False(backtest·production 단일 출처)·EARLY_GAIN_DAYS=21[B]·SELL_HALF_GAIN_PCT=0.20[B/D](구 backtest 리터럴 승격, TRADE_BREAKEVEN_TRIGGER_PCT 와 결합 금지). backtest 5B 산술 → 공용 순수 함수 sell_half.evaluate_sell_half. 동작 변화 0 — 표본 A+B OFF 1.0185 / ON 0.9448·절반매도 4 이전 실측과 동일. 의존성 맵 = docs/superpowers/plans/2026-09-08-issue166-sell-half-parity.md §4. ON 전환은 별도 사전등록(꼬리 절단 비용 명기).
