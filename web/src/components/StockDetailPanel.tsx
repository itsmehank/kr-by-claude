...
function TooltipBody({ hit, to }: { hit: ChartHit; to: string }) {
  ...
  if (hit.kind === "dot") {
    return (
      <div className="flex flex-col gap-1">
        ...
        <div className="text-data-xs text-muted">decision {hit.decision ?? "—"}</div>
        <div className="num text-data-xs">
          close {hit.close != null ? hit.close.toLocaleString() : "—"} · pivot{" "}
  ...
  if (hit.kind === "step") {
    return (
      <div className="flex flex-col gap-1">
        <span className="font-semibold num">pivot {hit.pivot.toLocaleString()}</span>
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <span className="text-data-xs text-muted">
        {hit.closed_by == null
          ? ongoingLabel(to)
          : `닫힘 · ${hit.closed_by === "disqualify" ? "실격" : "ignore"}`}
      </span>
    </div>
  );
}
...
export default function StockDetailPanel({ row, to }: { row: StockRow; to: string }) {
  ...
  function onMouseMove(e: React.MouseEvent) {
    ...
    const s = VB_W / rect.width; // 종횡비 고정 → 배율 하나(검토 #1)
    const mx = (e.clientX - rect.left) * s - PAD_X;
    const my = (e.clientY - rect.top) * s - PAD_Y;
  ...
  return (
    <div className="mb-6 rounded-xl border border-hairline bg-cream/70 p-4 flex gap-4 h-[380px]">
      <div
        ref={wrapRef}
        className="relative flex-[2] min-w-0 flex items-center"
