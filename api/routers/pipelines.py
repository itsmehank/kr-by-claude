...
def get_pipeline_detail(pipeline_id: str, conn: Connection = Depends(get_conn)):
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            SELECT id, mode, status, started_at, finished_at, rows_affected, total_count, details, error
              FROM pipeline_runs
             WHERE pipeline = %s
    ...
    recent_runs = []
    for row in rows:
        run_id, mode, status, started, finished, rows_affected, total_count, details, error = row
