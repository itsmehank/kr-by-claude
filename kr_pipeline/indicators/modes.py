...
def _process_ticker_weekly(
    ...
    adj_volume = df["adj_volume"]
    avg_vol_10w = avg_volume(adj_volume, window=10, min_periods=8)  # 주봉 halt ≤2주 허용 (min_periods=설계판단, book 근거 아님)
    ...
    w52h, w52l = w52_high_low(df["adj_high"], df["adj_low"], window=52, min_periods=50)  # 주봉 정지 ≤2주 허용 (min_periods=설계판단, book 근거 아님)
    pct_h, pct_l = pct_from_high_low(adj_close, w52h, w52l)
    rs_line = compute_rs_line(adj_close, df["index_close"])
