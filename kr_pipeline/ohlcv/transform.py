...
def nullify_halt_adj(df: pd.DataFrame) -> pd.DataFrame:
    ...
    adj_* 를 쓰는 *모든* 경로가 이 함수를 경유해야 한다 — ①daily INSERT(adjust.derive_adj, #207 A안)
    ②adj-refresh(_run_full_refresh → adjust.derive_adj) ③드리프트 소급(adjust.apply_event 는 곱셈이라 NULL 유지)
    ④레거시 merge_raw_and_adjusted. 모든 경로. 한 경로라도 누락하면 다음 적재가 halt 행을 0 으로 되돌린다(weekly 는 daily
