...
### 전문가 최종 사인오프 + 2건 ★확인 (2026-06-15)
- **P-B(하위 leg 미발화)**: 책-정합 옳음. climax(종말 소진→ignore) vs late-stage extended(상승
  계속→watch). ★확인 결과 298040 Jan/Apr 2차 leg(01-17·04-25)가 **`late_stage_base` 달림 →
  ×0.7 사이징** = "+400% 후기 leg 작게 타기" nuance 완벽 작동. 각 leg force-ignore 면 watch-기아 재도입.
- **P-A(2)(저변동 wide_loose)**: ★확인 결과 009970 35주 **`wide_and_loose` 0건** → mis-demote
  없음. ignore 0이 못 보여주던 가림막 제거, **저변동 우려 완전 종결**.
- **P-C(08-02 marginal)**: 진짜 FP(LLM 임계 불완전 적용). 단 SK·298040 각 ~1건 = **종목-무관
  노이즈 floor → 오히려 무-과적합 증거**. 미세조정 안 쫓음(비결정성 교훈). 군집화하면 신호.
- **P-D(완결 범위)**: A(분류 품질) 완결 동의, §6.2 제거만 최하위 후속. **단 A 완결 + 과적합 닫힘
  ≠ 라이브 준비.** 진짜 다음 마일스톤 = **수익성 백테스팅(미시작)** — 그것도 비결정성 교훈 적용
  (1회→저장→패턴). 009970 주말 entry 2 = entry 경로 작동 표면화(SK entry 0과 무모순).
