...
def test_process_ticker_daily_distribution_exact_boundary(db):
    """(#20 리뷰 후속) 파이프라인 산술 경유 정확 -0.2% 경계 포함 검증.

    100.0 → 99.8 은 pct_change 부동소수로 -0.19999...% 가 되어 eps 보정 없이는
    경계 탈락(실측 ~25% miss 클래스). eps=1e-9 보정으로 True 여야 한다.
    """
    from datetime import date, timedelta
    from kr_pipeline.indicators.modes import _process_ticker_daily

    t = "DISTBND"
    start = date(2010, 9, 6)  # 별도 격리 구간 (월요일)
    days = []
    d = start
    while len(days) < 61:
        if d.weekday() < 5:
    ...
    with db.cursor() as cur:
        ...
        for i, day in enumerate(days):
            if i < 60:
                ...
                close, vol = 99.8, 1100  # 정확히 -0.2% + ratio 1.1
            cur.execute(
                """INSERT INTO daily_prices (ticker, date, open, high, low, close,
                       adj_close, adj_high, adj_low, adj_open, adj_volume, volume, value)
                   VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,1)""",
    ...
    try:
        ...
        with db.cursor() as cur:
            ...
            flag = cur.fetchone()[0]
        assert flag is True, (
            "정확 -0.2% 하락일이 부동소수 경계에서 탈락 — eps 보정 회귀 (#20 리뷰)"
        )
    finally:
        with db.cursor() as cur:
            cur.execute("DELETE FROM daily_indicators WHERE ticker=%s", (t,))
